...
describe("TaskReportService", () => {
  ...
  it("rejects a report with no parts", () => {
    ...
    expect(result.message).toBe(
      "Add at least one replaced part before saving.",
    );
  });
  ...
  it("queues a valid payload through storage", async () => {
    ...
    expect(result.message).toBe(
      "Report saved on this device and queued for sync.",
    );
  });
  ...
  it("rejects invalid fields and unsafe totals", () => {
    ...
    expect(
      service.validateAndBuildPayload(task, "acct-1", invalid).isValid,
    ).toBe(false);
    const unsafe = makeForm();
    unsafe.controls.parts
      .at(0)
      .controls.unitCost.setValue(Number.MAX_SAFE_INTEGER);
    expect(
      service.validateAndBuildPayload(task, "acct-1", unsafe).message,
