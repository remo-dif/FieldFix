...
function fixture() {
  const storage = {
    online$: new BehaviorSubject(false), pendingCount$: new BehaviorSubject(0),
    getTask: vi.fn().mockResolvedValue(task), enqueueReport: vi.fn().mockResolvedValue(undefined),
  };
  const account = { ensureAccount: vi.fn().mockResolvedValue('acct-1') };
  const page = new TaskReportComponent(storage as any, account as any, new TaskReportService());
  page.taskId = 't-1';
...
describe('TaskReportComponent', () => {
  it('loads a cached task and saves a valid report locally', async () => {
    ...
    row.patchValue({ sku: 'P-1', description: 'Valve', quantity: 2, unitCost: 4.25 });
    ...
    await f.page.submit();
    expect(f.storage.enqueueReport).toHaveBeenCalledWith(expect.objectContaining({ taskId: 't-1', totalCostCents: 850 }));
  ...
  it('keeps the form open when validation or local persistence fails', async () => {
    ...
    const row = f.page.parts.at(0);
    row.patchValue({ sku: 'P-1', description: 'Valve', quantity: 1, unitCost: 2 });
