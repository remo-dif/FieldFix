...
function fixture() {
  const tasks = {
    clear: vi.fn(),
    put: vi.fn(),
    getAll: vi.fn(() => ({ result: [task] })),
    get: vi.fn(() => ({ result: task })),
  };
  const reports = {
    clear: vi.fn(),
    add: vi.fn(),
  ...
  const tx = {
    objectStore: vi.fn((name: string) => (name === "tasks" ? tasks : reports)),
  };
...
describe("OfflineStorageRepository", () => {
  ...
  it("commits a queued report and reads pending reports", async () => {
    ...
    expect(f.reports.add).toHaveBeenCalledWith(
      expect.objectContaining({ id: "r-1", state: "queued", attempts: 0 }),
    );
  ...
  it("prevents account clearing while reports are unsent", async () => {
    const f = fixture();
    f.reports.getAll.mockReturnValue({ result: [report as any] });
    await expect(f.repository.clearAccountData()).rejects.toThrow(
      "Unsent reports",
    );
