...
describe('AccountSessionStateService', () => {
  ...
  it('notifies only on changed account keys from another tab and stops after unsubscribe', () => {
    ...
    const state = new AccountSessionStateService({ get: vi.fn(), set: vi.fn(), remove: vi.fn() } as any);
    const changed = vi.fn();
    const stop = state.listenForAccountChanges(changed);
    const emit = (key: string, oldValue: string | null, newValue: string | null) =>
      browser.dispatchEvent(Object.assign(new Event('storage'), { key, oldValue, newValue }));
