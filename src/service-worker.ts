...
import { claimNextReport, openFieldFixDb, sendReport, settleReport, SYNC_TAG } from './app/offline/offline-db';

declare const self: ServiceWorkerGlobalScope & { __WB_MANIFEST: Array<{ url: string; revision?: string | null }> };
interface FieldFixSyncEvent extends ExtendableEvent { tag: string }
...
registerRoute(
  ({ request, url }) => url.origin === self.location.origin &&
    ['script', 'style', 'font', 'image'].includes(request.destination) && !url.pathname.startsWith('/api/'),
  new CacheFirst({ cacheName: 'fieldfix-static-v1', plugins: [new CacheableResponsePlugin({ statuses: [200] })] }),
);
...
registerRoute(
  ...
  new NetworkFirst({
    cacheName: 'fieldfix-task-data-v1', networkTimeoutSeconds: 3,
  ...
  event.waitUntil((async () => {
    ...
    try {
      while (true) {
        ...
        await settleReport(db, report.id, owner, outcome,
          outcome === 'conflict' ? 'Conflict or request rejected by the server' : undefined);
        // Notify open tabs that the report count has changed.
        const windows = await self.clients.matchAll({ type: 'window', includeUncontrolled: true });
        for (const client of windows) client.postMessage({ type: 'fieldfix-queue-changed' });
